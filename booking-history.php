  ...
  <table class="products-table">
    ...
    <tbody>
      <?php
        ...
        $query = mysqli_query($con, "SELECT UserID, AptNumber, AptDate, AptTime, Message, BookingDate,
        IFNULL(Remark, 'waiting for response') as Remark, 
        IFNULL(Status, 'waiting for response') as Status,
        IFNULL(RemarkDate, 'waiting for response') as RemarkDate
        FROM tblbook WHERE tblbook.UserID = '$userid'");
        if (!$query) {
          ...
          while ($row = mysqli_fetch_array($query)) {
            ...
            <tr>
              ...
              <?php if ($row['Remark'] == 'waiting for response') { ?>
                <td style="color: green; border-color: green;"><?php echo $row['Remark']; ?></td>
              <?php } else { ?>
                <td><?php echo $row['Remark']; ?></td>
              <?php } ?>
                            <?php if ($row['RemarkDate'] == 'waiting for response') { ?>
                <td style="color: green; border: 2px solid green"><?php echo $row['RemarkDate']; ?></td>
              <?php } else { ?>
              ...
              <?php }  elseif ($row['Status'] == 'waiting for response') { ?>
                <td style="color: green; border: 2px solid green;"><?php echo $row['Status']; ?></td>
              <?php } else { ?>
